...
RECIPES: Dict[str, Tuple[str, str, str]] = {
    ...
    "decorator": (SYNTAX, "`@decorator` on a `class`, on `async def` or before anything but a `def` (a decorated `def` is served)",
                  "decorate plain `def`s only"),
    ...
    "kwonly": (SYNTAX, "a keyword-only parameter list CPython rejects (`def f(*)`, `def f(*, a, /)`); `def f(*, a)` itself is served",
               "write keyword-only parameters after one `*`, each a name"),
