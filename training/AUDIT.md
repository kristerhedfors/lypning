...
Six independent lenses over `training/pipeline/`, every finding then handed to two
...
**`data-integrity/broken-corpus-cases-unpassable`** — `/home/user/lypning/training/pipeline/lypning_source.py:160`  
...
**`data-integrity/engine-mismatch-scored-as-model-failure`** — `/home/user/lypning/training/pipeline/evaluate.py:263`  
...
**`data-integrity/hashseed-nullified-by-dash-E`** — `/home/user/lypning/training/pipeline/sandbox.py:229`  
...
**`data-integrity/literal-min-chars-hole-plus-shortest-wins`** — `/home/user/lypning/training/pipeline/sample.py:190`  
...
**`end-to-end/hashseed-nullified-by-dash-e`** — `/home/user/lypning/training/pipeline/sandbox.py:229`  
...
**`end-to-end/literal-output-false-positives`** — `/home/user/lypning/training/pipeline/sample.py:59`  
...
**`end-to-end/refused-relabelled-wrong-output`** — `/home/user/lypning/training/pipeline/classify.py:57`  
...
**`gates-and-freeze/discriminates-gate-accepts-an-approximation`** — `/home/user/lypning/training/pipeline/acceptance.py:325`  
...
**`gates-and-freeze/pythonhashseed-defeated-by-dash-E`** — `/home/user/lypning/training/pipeline/sandbox.py:227`  
...
**`gates-and-freeze/stable-gate-cannot-see-an-unsatisfiable-test`** — `/home/user/lypning/training/pipeline/acceptance.py:341`  
...
**`lypning-kind/engine-mismatch-counted-as-a-model-failure`** — `training/pipeline/acceptance.py:410`  
...
**`lypning-kind/eval-path-has-no-literal-output-guard`** — `training/pipeline/acceptance.py:380`  
...
**`lypning-kind/tooling-cases-become-unpassable-rewrite-targets`** — `training/pipeline/lypning_source.py:86`  
...
**`sandbox/capture-files-inside-the-program-cwd`** — `training/pipeline/sandbox.py:216`  
...
**`sandbox/dash-E-defeats-pythonhashseed`** — `training/pipeline/sandbox.py:229`  
...
**`statistics/partial-run-wins-the-baseline-comparison`** — `training/pipeline/cli.py:522`  
...
**`statistics/results-ignores-sampling-config`** — `training/pipeline/cli.py:513`  
...
**`data-integrity/literal-guard-defeatable-and-absent-from-eval`** — `/home/user/lypning/training/pipeline/sample.py:54`  
...
**`data-integrity/power-table-keyed-by-a-lift-the-simulation-does-not-deliver`** — `/home/user/lypning/training/pipeline/stats.py` (`_treated`)  
`_treated` applies a uniform lift as `min(1.0, p + delta)` and returns the realised mean as a lift per CASE, while the `EVAL2.md` §4 rule the curve prices macro-averages over FAMILIES. `power_curve_clustered` returned neither realised figure in the rule's unit, and `cli._power_eval2` printed only the nominal `delta` under a column headed `effect` — so the docstring's own promise, that a cell is priced at the effect it tested, held only for a pilot with room for the whole lift and equal-sized families.

> Deflates the apparent sensitivity of the one rule eval-2 is sized by, and did: `EVAL2.md` §7 concluded from it that "a uniform few-point lift, the shape a small SFT most plausibly produces, cannot be seen by this instrument at N = 300 and k = 16", and round-02 was read under that sentence. CONFIRMED on synthetic pilots where the answer is known (2026-09-16, this tree, `power_curve_clustered`, k=16, N=300, mde +3pp, 60 trials, 200 resamples, default seed): on 64 single-case families with 45 at rate 1.0 and 19 at 0.0, a uniform lift asked at +8pp realises +2.4pp and fires 0% of the time, while the concentrated shape at the same bank realises its +5pp and fires 92% and its +8pp and fires 100% — the uniform rows were labelled by a treatment the bank had no room to receive. Separately, and independently of the cap, the per-case and macro realisations are different numbers on unequal families: on one 40-case family at 0.0 plus 24 singletons at 1.0 (64 cases, 25 families, same settings), a uniform +10pp reads +6.2pp per case where the macro the rule tests reads +0.5pp, and the concentrated shape reads +10.0pp per case against +0.7pp macro with the cap never biting at all. A table keyed on the per-case figure would report the rule blind to effects it detects. The eight calibration rows in `ASSESSMENT.md` §3.4 escape this because each of their pilots is 64 single-case families, where the two columns coincide by construction; whether the published §7 rows do is what rung S0a's re-print, keyed on the macro, will show. The concentrated shape clips too — its reachable lift PER CASE is at most `fraction × (1 − mean(lifted decile))`, so at fraction 0.1 a nominal +10pp arrives in full only on a decile at exactly 0; the macro is not bounded by that expression. Fixed the same day: `mean_macro_effect` and a `capped` flag are returned and rendered, §7's uniform reading is withdrawn, and `training/tests/test_eval2_power.py` pins that power tracks the macro.

**`data-integrity/prompt-signature-blind-to-render-contract`** — `/home/user/lypning/training/pipeline/evaluate.py:78`  
...
**`end-to-end/stable-gate-is-vacuous-for-rewrite-cases`** — `/home/user/lypning/training/pipeline/acceptance.py:344`  
...
**`gates-and-freeze/lock-hashes-provenance-so-enrichment-bricks-the-eval`** — `/home/user/lypning/training/pipeline/split.py:96`  
...
**`gates-and-freeze/no-literal-output-guard-in-evaluate`** — `/home/user/lypning/training/pipeline/evaluate.py:256`  
...
**`gates-and-freeze/sample-train-cases-skips-split-verify`** — `/home/user/lypning/training/pipeline/sample.py:70`  
...
**`lypning-kind/refusal-contract-is-never-actually-checked`** — `training/pipeline/acceptance.py:403`  
...
**`sandbox/unshare-runtime-failure-scored-as-the-program`** — `training/pipeline/sandbox.py:220`  
...
**`statistics/paired-delta-drops-cases-silently`** — `training/pipeline/stats.py:101`  
...
**`statistics/pass-at-k-draw-weighted-vs-case-weighted`** — `training/pipeline/stats.py:141`  
...
**`statistics/significant-is-one-sided`** — `training/pipeline/stats.py:123`  
...
**`data-integrity/extract-empty-fence-returns-empty-string`** — `/home/user/lypning/training/pipeline/extract.py:58`  
...
**`sandbox/process-group-kill-is-not-a-containment`** — `training/pipeline/sandbox.py:287`  
...
**`sandbox/truncation-is-never-detected`** — `training/pipeline/sandbox.py:135`  
...
**`statistics/mcnemar-on-fractional-means-is-not-mcnemar`** — `training/pipeline/stats.py:113`  
...
**`statistics/slices-bootstrap-depends-on-dict-order`** — `training/pipeline/cli.py:617`  
