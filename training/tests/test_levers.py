...
def test_a_status_filter_that_matches_no_draw_publishes_no_vector(draw_tree, capsys):
    ...
    assert "no refusal to publish" in out.err and "1 draw row(s) loaded" in out.err
    assert out.out == ""
...
def test_an_engine_that_runs_but_is_not_lypning_publishes_no_vector(draw_tree, capsys):
    ...
    assert "no refusal to publish" in out.err
    assert out.out == ""
...
def test_a_join_on_the_wrong_key_is_not_a_census_without_refusals(tmp_path, capsys):
    ...
    assert code == 1
    assert "1 match status correct-fallback; 1 have no replay row" in out.err
    assert out.out == ""
    code, out = _run_cli(base + ["--replay", str(bare)], capsys)
    assert code == 1
    assert "1 match status correct-fallback; 0 have no replay row" in out.err
    assert out.out == ""
