...
python3 training/gpu/train_verified.py sft --plan --smoke --isolated-worker \
  --bundle "$ROUND/smoke/bundle.json" --engine "$LYPNING_L_BIN" --revision "$QWEN_REV" \
  --output "$ROUND/sft-smoke-plan"
python3 training/gpu/train_verified.py sft --smoke --isolated-worker \
  --bundle "$ROUND/smoke/bundle.json" --engine "$LYPNING_L_BIN" --revision "$QWEN_REV" \
  --output "$ROUND/sft-smoke"
python3 training/gpu/train_verified.py grpo --smoke --from-base --isolated-worker \
  --bundle "$ROUND/smoke/bundle.json" --engine "$LYPNING_L_BIN" --revision "$QWEN_REV" \
...
python3 - <<'EOF'
import json, os
cfg = json.load(open("training/round-02.example.json"))
