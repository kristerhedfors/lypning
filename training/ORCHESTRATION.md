...
| P14 / Claude | **2026-09-24: arm A's pilot fitted to the h200 smoke** (branch `claude/pilot-fit`, $0, no dispatch). Smoke HF job `6ab4582d6b030d633f68c90e` (Actions `35930577878`): no OOM; 256-seq call 44.8 s natural, 359.4 s full-length; SFT 7.29 / 4.22 s/step. Arm A no longer probes (`probe_contract` binds adapter and code, so arm C re-probes in its own job); SFT step 0 is base-dev's draws under a no-op-LoRA proof (`reuse_step_zero`); `PILOT_EVAL2` `separate`; `projection.py` gains a `realistic` reading. Re-run from the smoke's numbers: pilot / eval-2 job 422.6 / 271.4 min measured, 764.4 / 638.3 realistic, 1,002.8 / 807.5 upper, against a 648-min budget; 195.0 of the realistic pilot's minutes are the stated scoring constant; whole scoring waves would add 24.4 / 23.1 min, putting the eval-2 job at 661.4, over budget | Operator: the realistic pilot is over 720m unless pooled scoring beats 20.8 worker-s/draw; decide whether to measure scoring throughput before seed 1111's h200 job. Arm A's configuration is unchanged |
