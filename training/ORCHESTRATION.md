...
| P11 / Claude | **2026-09-22: S4 refactor before arm A's first seed, and the independent Step 2 review** (branch `claude/s4-training-refactor`, $0, no dispatch). Selector on the coverage macro against a paired case-clustered margin; Step 2 decision coverage-only and complete runs only; split seed fixed at 1111 for every training seed; arm A launches with GRPO dose 0; curriculum case floor (1,000 distinct target cases) and family floor; torch-reference kernel enforced before the weight pull, binding recorded; LoRA init reseeded per seed; `QWEN_REV` pinned, grade tokenizer included; arm C's recipe is 4 prompts × 8 generations and LR 1e-5 (the launcher keeps 4 generations so the probe stays comparable with seed 1111's; arm C passes 8, which needs a `round02.yml` edit), no-signal logged not aborted; `verifier_sha256` widened, so bundles re-prepare. Review: `reviews/2026-09-22-claude-step2-s4-review.md` | **Revise (prepare).** Codex: one coverage-only grade of `35767396604`, `s4-target-preflight` on it (expected refusal on the case floor), draw-coupling and provider-seed reads. Operator: full-split target rung (~10,840 requests, sharded) and dev-selection draws 4 vs 16 (`launch.py --dev-eval-draws` / `DEV_EVAL_DRAWS`, default 4; arm field `dev_eval_draws`). Each seed's GPU ceiling stays a separate decision. No GPU spend before all of these. Seed 1111's kernel is not open: it ran `7d2bb09`, before `kernel_state` (`0733ac3`), whose message records all 48 gated-delta-net layers on the torch reference |
| P12 / Claude | **2026-09-22/23: capture at all times, with attribution; arm-A plumbing** (branch `claude/capture-finetune-prep`, stacked on #112; $0, no dispatch). User scope now registers the Bash capture hook only; Stop/SessionStart never export into a repository that is not a lypning checkout; an inert user-scope hook is reported; write-then-run `.py` files and Codex rollouts (`lypning harvest --codex`, read-only) are harvested; every resolved model is journaled to `attribution.jsonl`, which outlives transcript expiry. `nt capture-export` builds a static-AST-filtered, contamination-checked capture tier: 179 tier-A distinct programs from the live log on 2026-09-22. `step2-control.yml` gains a sharded `full` rung and `step2-merge`; `round02.yml` carries the dev-draw and arm-C knobs | Operator: merge #112, then this; journal attribution with `lypning harvest --export --transcripts` before transcripts expire (the default cleanup is 30 days, assumed, not measured); install user-scope capture after the dry run; approve or decline the ~$22 `full` rung; decide whether Codex/GPT programs may train the target. Captured programs are not an arm-A input |
| P13 / Claude | **2026-09-23: Step 2 and Step 3 closed; arm A targets ready** (operator approved paid non-GPU work and merges; GPU runs held for the operator). Full rung over the 1,163 remaining train cases: $22.38616209 in two shards, one resumed after a transport error (#115); Step 2's rungs total $28.12751896. Grades found one engine mismatch; its public-log leak of one train case id was deleted and closed by #117; the parser fix is held in #118 until arm A's seeds finish. Merge `35912725289`: coverage native +2.59pp [−3.94, +9.74], correct −7.13pp — flat, no distillation route. `step3-pairs` `35918571219`: at most 257 pair prompts — no preference arm. Three target sets clear the S4 floor at all seeds | Operator: choose `target_arms`, SFT dose (300 steps shows 36% of rows) and `PILOT_DEV_EVAL_DRAWS` (4 or 16; draws measured independent, `35849854123`); approve the hardware smoke and the seed-1111 h200 job. Bundles re-prepare in that job |
| P14 / Claude | **2026-09-24: arm A's pilot fitted to the h200 smoke** (branch `claude/pilot-fit`, $0, no dispatch). Smoke HF job `6ab4582d6b030d633f68c90e` (Actions `35930577878`): no OOM; 256-seq call 44.8 s natural, 359.4 s full-length; SFT 7.29 / 4.22 s/step. Arm A no longer probes (`probe_contract` binds adapter and code, so arm C re-probes in its own job); SFT step 0 is base-dev's draws under a no-op-LoRA proof (`reuse_step_zero`); `PILOT_EVAL2` `separate`; `projection.py` gains a `realistic` reading. Evaluation now scores chunk i while chunk i+1 generates (`verified_evaluation.ScoringStage`; rows, order, witnesses and aborts byte-identical to the serial loop; `--serial-scoring` for diagnosis). Re-run 2026-09-24 from the smoke's numbers with overlapped scoring: pilot / eval-2 job 349.2 / 198.0 min measured, 574.0 / 453.4 realistic, 812.4 / 622.6 upper, 295.5 / 198.0 lower, against a 648-min budget (serial: 422.6 / 271.4, 764.4 / 638.3, 1,002.8 / 807.5, 368.8 / 271.4). 4.7 of the stated scoring constant's 195.0 pool minutes reach the realistic pilot's clock, which stays inside the budget up to 55.8 worker-s/draw; whole scoring waves add 0.6 / 0.0 min there | Operator: at the realistic reading the pilot fits split (574.0 of 648); the upper reading still does not. Pooled scoring throughput is still unmeasured, but it no longer decides the realistic fit unless it is about 2.7 times slower than stated. Arm A's configuration is unchanged |
| P15 / Claude | **2026-09-24: an engine-mismatch draw is a counted draw** (branch `claude/eval-mismatch-policy`, $0, no dispatch). Seed-1111 arm A (HF job `6ab52a686b030d633f68e503`, Actions `36008052722`) finished SFT and aborted in its base test arm on one base-model draw that reached a frozen `lypning-l` bug (fix held in draft #118). GPU evaluation and GRPO now score such a draw `engine-mismatch`, reward 0, neither correct nor native, witnessed in the stage's private `engine-mismatches.jsonl`; a run fails past 1% of its planned draws (GRPO: per run). Every other block still aborts. `code_sha256` moves, `verifier_sha256` does not; arm A seed 1111 is re-run, its saved adapters cannot be carried over (`split_eval2` refuses a changed `code_sha256`). `EVAL2.md` §4 amendment | **Codex to rule:** a native TIMEOUT after a correct oracle is raised under the same kind and still aborts, keeping T4; counting it too would reverse T4. File the #118 bug from the private witness, never from the count |
| P16 / Claude | **2026-09-25: seed 1111 finished from its saved adapter at step 1,050** (branch `claude/finish-stage`, $0, no dispatch). Operator decision: evaluate SFT step 1,050 rather than the rule's 350, on pooled dev evidence (+1.9/+2.6/+3.1pp correct-native at 350/700/1,050), and finish rather than re-train. New `finish` stage (`round02_finish.sh`, `finish_lineage.py`, `finish_preflight.py`): pilot's 7f/7g with one sealed adapter, engine at the pilot's Space revision, every identity field refused by name before a load, `code_sha256` allowed to differ and recorded with the commits between. Override registered in `finish_lineage.OVERRIDES`; `arm_check` joins pilot and finish as one seed. Selector rule v2 `coverage-case-weighted/2` is the default for new selections (null 10.50%/9.18% at κ 2/20). Projection 445.2 of 648 min. `EVAL2.md` §4 amendment of 2026-09-25, `PLAN.md` Step 4 | **Operator:** dispatch `stage: finish` (`finish_of` 6ab52a686b030d633f68e503, `sft_step` 1050) after merge. **Codex to review** the override and rule v2 before seeds 2222/3333 are billed |
