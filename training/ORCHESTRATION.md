...
| R5 / Codex | **Closed 2026-09-17** in `reviews/2026-09-17-codex-s0-guards-and-engine-identity.md`: **continue**. All three requests ruled, plus the fourth the report flagged. D1 accept-with-conditions — Fable was entitled to make them, since each change only converts an output into a refusal and none completed the round; the boundary for next time is that a change to what a rung *reports* comes to Codex first. D2 accept-with-conditions — S0b is re-assigned to the frozen-rows form and is **not** held on a rebuild at `2e079e786a655ab6`, which folds in a core `lypning` sha no document records; 171 stands as a count at that identity and stops being re-derivable. D3 — the exact floor stays in `run()`, and `--plan` gains a byte-derived `supervised_token_upper_bound`; the shipped substitute `steps × batch_size` counted exposures, not tokens, and would have refused the runbook's own schedule. D4 — add a per-host density ceiling, not a worker floor: `16/16/1` was admitted and `1/1/1` is the *least* contended shape, so the hole was a missing ceiling. Verification found four still-open vacuous-read routes in `levers`, reproduced in-tree, now closed. Eight residual risks recorded; two parked as future calls (`power --eval2` rows provenance; `native` tested before `correct` in `eval2_rows`) | Fable runs S0a–S0c on the private device under the revised `START_NEXT_ROUND.md`, prints the grading identity beside the S0b vector, and stops. No paid rung, GPU job, Space rebuild or dataset mutation is authorised |
