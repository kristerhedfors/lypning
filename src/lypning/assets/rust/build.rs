...
fn main() {
    ...
    // Whether the version above was MEASURED — handed down by `lypning build`
    // out of `engines.find_cpython()`, or asked of a python here — or is the
    // fallback guess. `sys.version_info` is answered only in the first case
    // (`route::cap_attr`): a guessed minor is a wrong number at exit 0.
    let measured = named.is_some();
    let ref_py = named.unwrap_or_else(|| REF_PY_FALLBACK.to_string());
    println!("cargo:rustc-env=LYPNING_REF_PY={ref_py}");
    println!("cargo:rustc-env=LYPNING_REF_PY_MEASURED={}", measured as u8);
