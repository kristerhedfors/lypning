...
| `statistics` | `mean` `median` `median_high` `median_low` | `statistics.rs:SERVED` — `mean` over ints and bools only; a float `mean`, empty data and the spread functions are CPython's |
