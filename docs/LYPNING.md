...
| `lypning-l` | the same crate with `cap-collections`, `cap-csv`, `cap-glob`, `cap-hashlib`, `cap-pathlib` and `cap-re` (`engines.VARIANT_CAPS`), budgeted 32 blocks | `--features variant-l` |
...
| `src/collections.rs`, `src/csv.rs`, `src/glob.rs`, `src/hashlib.rs`, `src/pathlib.rs`, `src/re.rs` | `cap-collections`, `cap-csv`, `cap-glob`, `cap-hashlib`, `cap-pathlib`, `cap-re` — compiled into `lypning-l` only; `glob`'s ORDER half is a static blocker in `route.rs`, not a value in `glob.rs`, and `hashlib`'s object is a `Value::IterObj` over an `Iter`, not a `Value` variant of its own |
