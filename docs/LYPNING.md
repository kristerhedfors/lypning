...
| `lypning-l` | the same crate with `cap-collections`, `cap-csv`, `cap-pathlib` and `cap-re` (`engines.VARIANT_CAPS`), budgeted 32 blocks | `--features variant-l` |
...
| `src/collections.rs`, `src/csv.rs`, `src/pathlib.rs`, `src/re.rs` | `cap-collections`, `cap-csv`, `cap-pathlib`, `cap-re` — compiled into `lypning-l` only |
