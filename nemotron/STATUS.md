# Training programme status — 2026-09-15
...
| 3 | **Build eval-2** (`LADDER.md` stage 4): ≥300 unconditioned ordinary-task prompts, no mention of lypning, own lock and `prompt_sha`, both arms baselined before any adapter | Fable authors, Codex reviews | ~$40 | the only benchmark on which a positive result is a deployment claim |
...
does not start until 1, 3 and 5 are closed. The `nemotron/` directory rename is
